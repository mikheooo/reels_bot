...
async def test_call_gemini_api_429_retry_success(mock_sleep, mock_next_key, mock_get_keys):
    ...
    route = respx.post(url, headers={"x-goog-api-key": "TEST_KEY_1"})
    route.side_effect = [
...
async def test_call_gemini_api_key_rotation_on_429(mock_sleep, mock_get_keys):
    url_a = f"https://generativelanguage.googleapis.com/v1beta/models/{TARGET_MODEL}:generateContent"
    url_b = url_a
    
    respx.post(url_a, headers={"x-goog-api-key": "KEY_A"}).mock(return_value=httpx.Response(429, text='{"error": "Rate limit"}'))
    respx.post(url_b, headers={"x-goog-api-key": "KEY_B"}).mock(return_value=httpx.Response(200, json={"candidates": [{"content": {"parts": [{"text": "key_b_success"}]}}]}))
...
async def test_get_raw_transcript_429_retry(mock_sleep, mock_delete, mock_get_file, mock_upload):
    mock_file = MagicMock()
    mock_file.name = "files/test_vid_123"
    mock_file.uri = "https://files.example/test_vid_123"
    ...
    url = f"https://generativelanguage.googleapis.com/v1beta/models/{TARGET_MODEL}:generateContent"
    route = respx.post(url, headers={"x-goog-api-key": "KEY1"})
    route.side_effect = [
        httpx.Response(429, text='{"error": "rate limited"}'),
        httpx.Response(
            200,
            json={"candidates": [{"content": {"parts": [{"text": "Transcribed text from video"}]}}]},
        ),
    ]
    ...
    with patch.dict("os.environ", env_vars), \
         ...
         patch("app.core.config.settings.transcription_primary_enabled", False):
